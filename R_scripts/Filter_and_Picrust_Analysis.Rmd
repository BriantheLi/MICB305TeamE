---
title: "Filtering and Analysis of Alcohol and IBD patients"
...
health = ryan_ibd_merged_data |> filter(Condition == "Ulcerative Colitis")
```

# Figure 1
```{r data for figure 1 panel A-D}
...
# Phyloseq obj for alpha diversity for alcohol dataset
...
# Alpha shannon diversity for high vs low alcohol consumption
## Panel A
```{r}
...
# Saving the plot
ggsave(
  filename = "Figure_1_Panel_A_alcohol_shannon.png",
  plot = shannon_alcohol_plot,
  path = "../Results/Figures",
  width = 18,
  height = 12)
```

```{r Statistical test for alpha shannon diversity for alcohol dataset}
...
```{r Preparing the phyloseq object}
# counts
...
# Saving the plot
ggsave(
  filename = "Figure_1_Panel_B_IBD_shannon.png",
  plot = shannon_ibd_plot,
  path = "../Results/Figures",
  width = 18,
  height = 12)
```

```{r Statistical test for alpha shannon diversity for IBD}
# Kruskal-Wallis test
...
# Jaccard for high vs low alcohol consumption
## Panel C
```{r}
...
# Plot alcohol Jaccard diversity
alcohol_plot = mds_data_alcohol %>%
  ...
  scale_colour_manual(values = c("Low" = "#A6CEE3",
                                 "High" = "#CAB2D6")) +
  labs(colour = "Alcohol Consumption")

alcohol_plot

# Saving the plot
ggsave(
  filename = "Figure_1_Panel_C_alcohol_Jaccard.png",
  plot = alcohol_plot,
  path = "../Results/Figures",
  width = 18,
  height = 12)
...
```{r Statistical test for jaccard: high vs low alcohol consumption}
...
# Saving the plot
ggsave(
  filename = "Figure_1_Panel_D_ibd_Jaccard.png",
  plot = ibd_plot,
  path = "../Results/Figures",
  width = 18,
  height = 12)
...
```{r Statistical test for jaccard: IBD}
...
# Figure 2 Panel A and B - core microbiome
```{r Preparing data for core microbiome}
...
# Making phyloseq object for core microbiome
```{r}
# Making alcohol phyloseq objects

# alcohol counts datatset
...
# Plotting the microbime venn diagram
## Alcohol microbiome - Panel A
```{r}
# Rarefy if desired and convert to relative abundance
alc_psrare = rarefy_even_depth(alc_ps, sample.size = 9000, rngseed=123)
...
alcohol_venn_core_microbiome = ggVennDiagram(
    ...
    label_color = "white",
    label_size = 5,
    category.names = c('Low alcohol intake','High alcohol intake')) +
...
# Saving the plot
alcohol_venn_core_microbiome = alcohol_venn_core_microbiome +
  theme(text = element_text(size = 18))
ggsave(
  filename = "Figure_2_Panel_A_alcohol_venn_core_microbiome.png",
  plot = alcohol_venn_core_microbiome,
  path = "../Results/Figures",
  width = 16,
  height = 12,
  bg = "white")
...
## IBD microbiome - Panel B
```{r}
# Rarefy if desired and convert to relative abundance, use tax_glom (optional)
ibd_psrare = rarefy_even_depth(ibd_ps, sample.size = 4167, rngseed=123)
...
ibd_venn_core_microbiome = ggVennDiagram(
    ...
    label_color = "white",
    label_size = 5) +
  scale_fill_gradientn(colors = c("#CC79A7", "#C4D8F3"))
ibd_venn_core_microbiome

# Saving the plot
ibd_venn_core_microbiome <- ibd_venn_core_microbiome +
  theme(text = element_text(size = 22))
ggsave(
  filename = "Figure_2_Panel_B_ibd_venn_core_microbiome.png",
...
```{r Differential pathway analysis for alcohol intake and IBD}
# Alcohol
...
# As there are no significant differences between different alcohol inflammatory environments' high level pathways, there would not be
# any plot for the picrust of alcohol
```
